...
import ${service.package}.${service.name}ResponderService;
...
public final class Consumer {
	...
	public static void main(final String[] args) {
		...
		final String adress = "${service.producer.url}";
	...
	public static String callService(final String serviceAddress, final String logicalAddresss) {
		...
		final ${service.name}ResponderInterface service = new ${service.name}ResponderService(
			Consumer.createEndpointUrlFromServiceAddress(serviceAddress)).get${service.name}ResponderPort();

		final AttributedURIType logicalAddressHeader = new AttributedURIType();
		logicalAddressHeader.setValue(logicalAddresss);

		final ${service.name}Type request = new ${service.name}Type();
		 ...
		 */
		final ${service.name}ResponseType result = service.${service.method.name}(logicalAddressHeader, request);
