...
public class PayloadToStringTransformer {
	...
	//
	public PayloadToStringTransformer(JAXBContext jaxbContext) {
	...
	private String getContentAsString(Object payload) {
		...
		if (payload instanceof String) {
			...
			content = XmlUtil.convertReversibleXMLStreamReaderToString((ReversibleXMLStreamReader) payload, "UTF-8");
	...
	private static String convertJmsMessageToString(Object payload, String outputEncoding) {
		try {
			return JmsMessageUtils.toObject((Message) payload, null, outputEncoding).toString();
		} catch (Exception e) {
