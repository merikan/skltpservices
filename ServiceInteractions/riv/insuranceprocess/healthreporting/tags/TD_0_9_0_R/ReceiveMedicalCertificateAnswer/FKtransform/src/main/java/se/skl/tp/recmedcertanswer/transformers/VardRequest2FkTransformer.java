...
public class VardRequest2FkTransformer extends AbstractMessageAwareTransformer
{
	...
	public Object transform(MuleMessage message, String outputEncoding) throws TransformerException {
		...
		try {			
    		...
    		// Mottagare - FK
    		FkAdresseringsType inMottagare = inRequest.getAnswer().getAdressFK();
    		...
    		outAmne.setFritext(inRequest.getAnswer().getMeddelanderubrik());
    		outTaEmotSvar.setAmne(outAmne);
    		
    		// Besvararas ?? Same as besvaras date?
//    		outTaEmotSvar.setBesvaras(inRequest.getAnswer().getSistaDatumForKomplettering());
			...
			outMeddelande.setText(inRequest.getAnswer().getSvar().getMeddelandeText());
			outMeddelande.setSignerades(inRequest.getAnswer().getSvar().getSigneringsTidpunkt()); 
			outTaEmotSvar.setSvar(outMeddelande );
