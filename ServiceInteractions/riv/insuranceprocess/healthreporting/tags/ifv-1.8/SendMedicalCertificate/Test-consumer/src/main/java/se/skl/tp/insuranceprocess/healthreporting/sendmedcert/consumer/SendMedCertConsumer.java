 ...
 */
package se.skl.tp.insuranceprocess.healthreporting.sendmedcert.consumer;
...
import se.skl.riv.insuranceprocess.healthreporting.sendmedicalcertificate.v1.rivtabp20.SendMedicalCertificateResponderService;
...
public final class SendMedCertConsumer {

	// Use this one to connect via Virtualiseringsplattformen
	private static final String LOGISK_ADDRESS = "/SendMedicalCertificate/1/rivtabp20";
	...
	public static void main(String[] args) {
		...
		String p = callSendMedicalCertificate("RIV TA BP2.0 Ref App OK", adress, "FKTransform");
		System.out.println("Returned: " + p);
	}

	public static String callSendMedicalCertificate(String id, String serviceAddress,
			String logicalAddresss) {

		SendMedicalCertificateResponderInterface service = new SendMedicalCertificateResponderService(
				createEndpointUrlFromServiceAddress(serviceAddress)).getSendMedicalCertificateResponderPort();
		...
		try {
			// Set an answer 
			request.setSend(getSendData());

			SendMedicalCertificateResponseType result = service.sendMedicalCertificate(logicalAddressHeader, request);
	...
	private static SendType getSendData() throws Exception {
		SendType meddelande = new SendType();
		...
		meddelande.setVardReferensId("varuniqueguid");
